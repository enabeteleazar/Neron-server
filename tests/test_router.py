# tests/test_router.py - mis a jour avec TIME_QUERY
...
@pytest.mark.asyncio
async def test_time_query_heure(router):
result = await router.route(“quelle heure est-il”)
assert result.intent == Intent.TIME_QUERY
assert result.confidence == “high”

@pytest.mark.asyncio
async def test_time_query_date(router):
result = await router.route(“quelle est la date”)
assert result.intent == Intent.TIME_QUERY
assert result.confidence == “high”

@pytest.mark.asyncio
async def test_time_query_jour(router):
