...
from web_bridge import registry_runner


def test_web_registry_payload_uses_common_sdk():
...
@pytest.mark.asyncio
async def test_web_sidecar_starts_and_stops_registry_client():
    client = Mock()
    client.start = AsyncMock(side_effect=lambda: stop_event.set())
...
@pytest.mark.asyncio
async def test_web_sidecar_survives_core_unavailability():
