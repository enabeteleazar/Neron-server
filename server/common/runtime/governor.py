...
class RuntimeEvent(Protocol):
    """Ce que le gouverneur attend d un evenement, et rien de plus.

    Le noyau partage ne doit connaitre AUCUN type de plateforme metier : il
    importait auparavant `core.infrastructure.event_bus.Event`, ce qui faisait
    dependre le noyau de Core. Le gouverneur ne lit en realite que deux
    attributs — c est donc un contrat structurel, exprime ici. N importe quel
    evenement portant `type` et `payload` convient, d ou qu il vienne.
    """
...
class RuntimeGovernor:
    ...
    async def handle_self_model_event(self, event: RuntimeEvent) -> None:
...
async def handle_self_model_governor_event(event: RuntimeEvent) -> None:
