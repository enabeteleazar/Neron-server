...
from server.common.registry.models import (
    RegistryService,
    RegistrySettings,
    service_from_env,
)
...
class RegistryClient:
    """Reusable, non-blocking Core Registry client for NéronOS services."""
    ...
    def __init__(
        ...
        *,
        ...
        env_settings = RegistrySettings.from_env(
            default_heartbeat_interval=heartbeat_interval or 30.0,
            default_timeout=timeout,
            default_initial_backoff=initial_backoff,
        )
        self.settings = RegistrySettings(
            core_url=(core_url or env_settings.core_url).rstrip("/"),
            api_key=env_settings.api_key if api_key is None else api_key,
            heartbeat_interval=(
                env_settings.heartbeat_interval
    ...
    async def register(self) -> bool:
        ...
        if success:
            ...
            self._logger.info(
                "Registered %s with Core Registry at %s",
                self.service.service_name,
                self.settings.core_url,
            )
        return success
