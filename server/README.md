...
```bash
cd /srv/homelab/server-1/neronOS
...
L'unité utilise `/srv/homelab/server-1/neronOS/server` comme `WorkingDirectory` et comme
`PYTHONPATH`, avec la commande suivante :

```text
/srv/homelab/server-1/neronOS/venv/bin/python -m uvicorn core.app:app --host 0.0.0.0 --port 8010
```

Les chemins runtime sont centralisés dans `common.paths`. `NERON_ROOT`
vaut `/srv/homelab/server-1/neronOS` par défaut et `NERON_CONFIG` vaut
